...
(function(win) {
  ...
  var ieVersion = doc.querySelector ? doc.documentMode : doc.compatMode ===
    "CSS1Compat" ? xhr in win ? 7 : 6 : 5;
  ...
  var js_path = (doc.scripts ? doc.scripts[doc.scripts.length - 1] : doc.querySelector(
    "script:last-child")).getAttribute("src").replace(/[^\/]+$/, "");
  var ajaxCache = {};
    ...
    if (!win.StyleFix) {
      loadScript(js_path + "prefixfree.min.js").onload = function() {
        ...
        PrefixFree.properties.forEach(function(property) {
          stylePropertyFix(StyleFix.camelCase(property), PrefixFree.prefixProperty(
            property, true));
        });
      };
    }
  ...
  // PIE
  var pie_path = win.PIE && "behavior" in PIE ? PIE.behavior : js_path.replace(
    RE_ORIGIN, "") + "PIE.htc";
  ...
  function setLengthUnits() {
    ...
    forEach(function(stylesheet) {
      ...
      if (cssText) {
        ...
        if (ieVersion < 9) {
          cssText = cssText.replace(/\b(\d+(\.\d+)?)rem\b/g, function(s,
            num) {
  ...
  function patchStyleSheet(cssText) {
    ...
    cssText = cssText.replace(RE_MEDIA, function(str, strRules) {
      return "@media " + strRules.replace(
        /\s+and\s+\(\s*(\w+\-)?msie\s*:\s*([\d\.]+)\s*\)/g,
        function(s, cond, ver) {
          ver = parseFloat(ver);
          if (cond) {
            if (/^max/.test(cond)) {
    ...
    cssText = cssText.replace(
      /{(([^{}]*)\bbackground(-\w+)?\s*:\s*(\w+-gradient\s*\([^;\}]+))/g,
      function(str, props, propsPre, backSubVal, gradient) {
        return /background(-image)?\s*:[^;]*url\(/g.test(propsPre) ? str :
          "{" + pie_path + "-pie-background:" + gradient + ";" + props;
      }).replace(
      /{(?=[^{}]*\bborder-image\s*:[^{}]+})/g,
      "{" + pie_path
    );
    if (ieVersion < 9) {
      ...
      cssText = cssText.replace(
        ...
        function(s, vals) {
          ...
          vals = trim(vals.split(/\s+(?=\w+\s*[\(\:])/).filter(function(
            filter) {
            return !
              /^(progid\s*\:\s*DXImageTransform\.Microsoft\.)?(Alpha|Matrix|Gradient|FlipH|FlipV)\s*\(/i
              .test(filter);
          }).join(" "));
    ...
    // IE CSS3 selector
    return ieVersion > 8 ? cssText : cssText.replace(RE_PSEUDO_ELEMENTS,
      PLACEHOLDER_STRING).
    replace(RE_SELECTOR_GROUP, function(m, prefix, selectorText) {

      var selectorGroups = [];
      forEach(function(selector, c) {
        selector = normalizeSelectorWhitespace(selector) +
          SPACE_STRING;
        var patches = [];
        selectorGroups[c] = selector.replace(RE_SELECTOR_PARSE,
          function(match, combinator, pseudo, attribute, index) {
            if (combinator) {
              ...
              var patch = (pseudo) ? patchPseudoClass(pseudo) :
  ...
  function patchAttribute(attr) {
    return (!BROKEN_ATTR_IMPLEMENTATIONS || BROKEN_ATTR_IMPLEMENTATIONS.test(
      attr)) ? {
    ...
    if (pseudo.charAt(0) === ":") {
      switch (pseudo.slice(1)) {
          ...
          if (ieVersion === 8) {
            applyClass = function(e) {
              var handler = function() {
                ...
                return isNegated ? (hash === EMPTY_STRING || e.id !==
                  hashID) : (hash !== EMPTY_STRING && e.id === hashID);
              };
            ...
            if (RE_INPUT_CHECKABLE_TYPES.test(e.type)) {
              addEvent(e, "propertychange", function() {
                if (event.propertyName === "checked") {
                  toggleElementClass(e, className, e.checked !==
                    isNegated);
                }
              });
            }
            ...
            if (RE_INPUT_ELEMENTS.test(e.tagName)) {
              addEvent(e, "propertychange", function() {
                if (event.propertyName === "$disabled") {
                  toggleElementClass(e, className, e.$disabled ===
                    isNegated);
                }
              });
        ...
        case "hover":
          ...
          applyClass = function(e) {
            addEvent(e, isNegated ? deactivateEventName :
              activateEventName,
              function() {
                toggleElementClass(e, className, true);
              });
            addEvent(e, isNegated ? activateEventName :
              deactivateEventName,
              function() {
  ...
  function applyPatches() {
    ...
    for (var c = 0; c < domPatches.length; c++) {
      ...
      domSelectorText = selectorText.replace(
        RE_LIBRARY_INCOMPATIBLE_PSEUDOS, EMPTY_STRING);

      // If the dom selector equates to an empty string or ends with
      // whitespace then we need to append a universal selector (*) to it.
      if (domSelectorText === EMPTY_STRING || domSelectorText.charAt(
          domSelectorText.length - 1) === SPACE_STRING) {
        domSelectorText += "*";
      }
        ...
        for (var d = 0, dl = elms.length; d < dl; d++) {
          ...
          for (var f = 0, fl = patches.length; f < fl; f++) {
            ...
            if (!hasPatch(elm, patch)) {
              if (patch.applyClass && (patch.applyClass === true || patch.applyClass(
                  elm) === true)) {
  ...
  function normalizeSelectorWhitespace(selectorText) {
    return normalizeWhitespace(selectorText.replace(
      RE_TIDY_TRAILING_WHITESPACE, PLACEHOLDER_STRING).replace(
      RE_TIDY_LEADING_WHITESPACE, PLACEHOLDER_STRING));
  }
  ...
  function toggleClass(classList, className, on) {
    ...
    if (on) {
      ...
      return classExists ? trim(classList.replace(re, PLACEHOLDER_STRING)) :
        classList;
    }
  }
  ...
  function resolveUrl(url, contextUrl, ignoreSameOriginPolicy) {
    ...
    // absolute path
    if (/^\w+:\/\//i.test(url)) {
      return !ignoreSameOriginPolicy && getProtocolAndHost(contextUrl) !==
        getProtocolAndHost(url) ? null : url;
    }
    ...
    if (url.charAt(0) !== "?" && contextUrlPath.charAt(contextUrlPath.length -
        1) !== "/") {
      contextUrlPath = contextUrlPath.substring(0, contextUrlPath.lastIndexOf(
        "/") + 1);
    }
  ...
  function parseStyleSheet(url) {
    if (url) {
      ...
      replace(RE_IMPORT, function(match, quoteChar, importUrl, quoteChar2,
        importUrl2, media) {
        var cssText = parseStyleSheet(resolveUrl(importUrl ||
          importUrl2, url));
        return (media) ? "@media " + media + " {" + cssText + "}" :
          cssText;
      }).
      replace(RE_ASSET_URL, function(match, isBehavior, quoteChar, assetUrl) {
        quoteChar = quoteChar || EMPTY_STRING;
        return isBehavior ? match : " url(" + quoteChar + resolveUrl(
          assetUrl, url, true) + quoteChar + ") ";
      });
    }
    return EMPTY_STRING;
  }
  ...
  function getStyleSheets() {
    ...
    forEach(function(styleSheet) {
      ...
      if (url && !(strRawCssText in styleSheet)) {
        styleSheet[strRawCssText] = patchStyleSheet(parseStyleSheet(
          resolveUrl(url) || url));
      }
    });
  ...
  if (ieVersion > 8) {
    ...
    if (ieVersion < 8) {
      pie_path =
        "behavior: expression(window.PIE&&PIE.attach_ie67&&PIE.attach_ie67(this));";
    } else {
